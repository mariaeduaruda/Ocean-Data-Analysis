...
def plot_boia(dataframe, categoria):

    dados_plot = dataframe.query('Categoria == @categoria')

    fig, ax = plt.subplots(figsize=(8,6))
    ax = sns.barplot(x = 'Lugar', y = 'Temperatura', data = dados_plot)
    ax.set_title(f'A {categoria} registrada em cada uma das Boias', fontsize = 16)
    ax.set_xlabel('Boia, Latitude e Longitude', fontsize = 12)
...
# importando os dados
dados = pd.read_csv('boia.csv')

st.title('Análise de Temperatura nas Boias\n')
st.write('Nesse projeto vou analisar as maiores e menores temperaturas registrdas por boias que monitoraramm o mar da Irlanda')
st.write('Os dados coletados são agosto de 2021 até o mês de agosto de 2022')
...
figura = plot_boia(dados, categoria_grafico)
st.pyplot(figura)
